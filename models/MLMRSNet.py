...
class MLMRSNet:
    ...
    def IMUNet(self):
        ...
        if self.A_E == 1:
            # Collect Latent Features or Embeddings from AutoEncoders
            pool = Feature_Extraction_Block(pool, self.model_width, self.feature_number)
        conv = MRP_Block(pool, self.model_width, 2 ** self.model_depth, pooling_type=self.pooling_type, cardinality=0, use_batchnorm=True)
        
        # For Deep Supervision
        if self.problem_type == 'Classification':
            level = tf.keras.layers.Conv1D(1, 1, activation='sigmoid', name=f'level{self.model_depth}')(conv)
        elif self.problem_type == 'Regression':
            level = tf.keras.layers.Conv1D(1, 1, activation='linear', name=f'level{self.model_depth}')(conv)
        levels.append(level)
        ...
        for j in range(0, self.model_depth):
            ...
            deconv_tot = MRP_Block(deconv, self.model_width, 2 ** 0, pooling_type=self.pooling_type, cardinality=0, use_batchnorm=True)
            ...
            deconv = MRP_Block(deconv_tot, self.model_width, 2 ** (self.model_depth - j), pooling_type=self.pooling_type, cardinality=0, use_batchnorm=True)
            deconvs["deconv%s" % j] = deconv
            if self.D_S == 1 and (j < (self.model_depth - 1)):
                # For Deep Supervision
                if self.problem_type == 'Classification':
                    level = tf.keras.layers.Conv1D(1, 1, activation='sigmoid', name=f'level{self.model_depth - j - 1}')(deconv)
                elif self.problem_type == 'Regression':
                    level = tf.keras.layers.Conv1D(1, 1, activation='linear', name=f'level{self.model_depth - j - 1}')(deconv)
                levels.append(level)
        # Output
        outputs = []
        if self.problem_type == 'Classification':
            outputs = tf.keras.layers.Conv1D(self.output_nums, 1, activation='sigmoid', name="out")(deconv)
        elif self.problem_type == 'Regression':
            outputs = tf.keras.layers.Conv1D(self.output_nums, 1, activation='linear', name="out")(deconv)
...
if __name__ == '__main__':
    ...
    model_name = 'IMUNet'  # UNet or UNetPP
